...
import numpy as np
from peft import LoraConfig, get_peft_model, TaskType  # New imports for LoRA
...
def main(args):
    ...
    # Configure LoRA specifically for Whisper
    lora_config = LoraConfig(
        #task_type=TaskType.FEATURE_EXTRACTION, # Pb with LoRA, so Github suggested: Remove task_type from LoraConfig, which helps me resolve this issue. You can find a solution at: https://discuss.huggingface.co/t/unexpected-keywork-argument/91356
        r=8,  # Rank
        lora_alpha=32,
    ...
    # Convert the model to PEFT model
    model = get_peft_model(model, lora_config)

    # Print trainable parameters
    model.print_trainable_parameters()

    # Define a custom data collator that handles the audio features correctly
    ...
    trainer = MyTrainer(
        ...
        data_collator=data_collator  # Add our custom collator
    )
